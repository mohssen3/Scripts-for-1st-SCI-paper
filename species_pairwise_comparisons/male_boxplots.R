...
s0<- per_timepoint("M",quote(Timepoint=="0dpi"|Timepoint=="7dpi"),"species") %>%
  filter(group2=="Lam")
unique(s0$taxon)

s1<- per_timepoint("M",quote(Timepoint=="7dpi"|Timepoint=="21dpi"),"species") %>%
  filter(group2=="Lam")
unique(s1$taxon)

s2<- per_timepoint("M",quote(Timepoint=="21dpi"|Timepoint=="35dpi"),"species") %>%
  filter(group2=="Lam")
unique(s2$taxon)

s3<- per_timepoint("M",quote(Timepoint=="35dpi"|Timepoint=="63dpi"),"species") %>%
  filter(group2=="Lam")
unique(s3$taxon)

s4<- per_timepoint("M",quote(Timepoint=="63dpi"|Timepoint=="180dpi"),"species") %>%
  filter(group2=="Lam")
unique(s4$taxon)

all_sig_taxa<-unique(bind_rows(s1,s2,s3,s4)$taxon) #for species
...
to_plot<- function(sex,taxonomy_level,Title){
  ...
  to_be_plotted<- taxon_relabund %>%
    ...
    summarize(pool=max(median) >=0.5,
              median=max(median)) %>%
    # summarize(pool=max(median) <=0.5 & max(median) >=0,
    #           median=max(median)) %>%
    # summarize(pool=max(median) <0.1,
    #           median=max(median)) %>%
    ungroup() %>%
    filter(pool==TRUE) %>%
    inner_join(.,taxon_relabund, by="taxon",multiple="all") %>%
    # mutate(rel_abund = ifelse(rel_abund == 0,0.000000001, rel_abund)) %>% #in case of max(median) <0.01 as 0 will not work when testing
    inner_join(short_metadata) %>%
    filter(Timepoint!="0dpi") %>%
    filter(taxon!="Acetatifactor sp910578185") #because it isn't significantly different when using the wilcoxon test upon plotting
...
p1<-to_plot("M","species","Significantly Changing Species with Highest Relative Abundances in Male Mice")
p1
ggsave("species_highrel_boxplot.pdf",p1,device = "pdf",width = 6.5,height = 15)
