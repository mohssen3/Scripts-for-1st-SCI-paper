...
to_plot<- function(sex,taxonomy_level,Title){
  ...
  to_be_plotted<- taxon_relabund %>%
    ...
    filter(Timepoint!="0dpi") #change for plot2 to ==0dpi
  ...
  p1<- 
    ...
    theme_classic()+
    geom_violin(aes(fill=Group),#linewidth=0.9,
...
ggsave("cowplot_phyla_male_mostabundant.pdf",cow1,device = "pdf",width = 6,height = 3)



cow2<-plot_grid(p4,p3, ncol=2, align = "v", axis="1")
cow2 #view the multi-panel figure 
ggsave("cowplot_phyla_female_mostabundant.pdf",cow2,device = "pdf",width = 6,height = 3)
